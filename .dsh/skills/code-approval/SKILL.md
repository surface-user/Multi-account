...
- AI 负责**说明改动内容**（改了哪些文件、为什么改）。
- **commit 信息只在用户认为合适时才补充**：默认不要每次代码改动后都自动给出 commit description；只有用户**明确需要/要求**时，才提供本次改动的 commit description（标题 + 正文，中文、遵循 Conventional Commits），供用户直接使用。绝不代劳提交、暂存或修改 commit 历史。
